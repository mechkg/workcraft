...
public class StgTextReferenceManager extends ExpressionBase<StgTextRefMan> {

	private final Expression<? extends NodeContext> nodeContext;
	private final Expression<? extends StgRefMan> referenceManager;

	public StgTextReferenceManager(Node root, Expression<? extends NodeContext> nodeContext, Expression<? extends StgRefMan> refMan, StgRefMan initialRefMan, References existingReferences) {
		this.nodeContext = nodeContext;
		this.referenceManager = refMan;
		
		if (existingReferences != null) {
			setExistingReference(initialRefMan, existingReferences, root);
			for (Node n: Hierarchy.getDescendantsOfType(root, Node.class))
				setExistingReference(initialRefMan, existingReferences, n);
		...
		if (node instanceof SignalTransition) {
			...
			try {
				...
				if (Identifier.isValid(s)) {
					refMan.setInstance(st, Pair.of(Pair.of(s, refMan.getInstance(st).getFirst().getSecond()), (Integer)null));
				} else
					throw new ArgumentException ("\"" + s + "\" is not a valid signal transition label.");
			}
	...
	@Override
	protected StgTextRefMan evaluate(final EvaluationContext context) {
		return new StgTextRefMan(){
			...
			private String getStgPlaceReference(STGPlace place) {
				if(context.resolve(place.implicit())) {
					...
					return context.resolve(referenceManager).getName(place);
			}

			private String getSignalTransitionReference(SignalTransition st) {
				Pair<Pair<String, Direction>, Integer> instance2 = context.resolve(referenceManager).getInstance(st);
			...
			public String getNodeReference(Node node) {
				if (node instanceof SignalTransition) {
					...
					return context.resolve(referenceManager).getMiscNodeName(node);
			}

			private String getDummyTransitionReference(
					final EvaluationContext context, final DummyTransition t) {
				final Pair<String, Integer> name = context.resolve(referenceManager).getInstance(t);
			...
			public Node getNodeByReference(String reference) {
				...
				if (implicitPlaceTransitions!=null) {
					...
					if (instancedSignalName != null) {
						...
						return context.resolve(referenceManager).getSignalTransition(Pair.of(Pair.of(instancedSignalName.getFirst(), instancedSignalName.getSecond()), instancedSignalName.getThird()));
					}
					...
					if (instancedName != null) {
						...
						Node n = context.resolve(referenceManager).getDummyTransition(instancedName);
						if (n!=null)
							return n;
					}
	
					return context.resolve(referenceManager).getMiscNode(reference);
				}
			}

			@Override
			public void setName(Node node, String s) {
				StgRefMan refMan = context.resolve(referenceManager);
