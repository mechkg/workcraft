...
object VisualConnectionGui {
  ...
  case class ExprConnectionGui(
      shape : Expression[Touchable]
      , graphicalContent : Expression[ColorisableGraphicalContent]
      , parametricCurve : Expression[ParametricCurve]) 
	def makeConnectionTouchable(curve : ParametricCurve) : TouchableC =
	  new TouchableC(new Touchable {
			override def hitTest(point : Point2D.Double) =
				curve.getPointOnCurve(curve.getNearestPointT(point)).distance(point) < HitThreshold
	...
	def getConnectionGui(properties : VisualConnectionProperties, context : VisualConnectionContext, data : StaticVisualConnectionData) = {
		...
		val touchable = makeConnectionTouchable(curve)
