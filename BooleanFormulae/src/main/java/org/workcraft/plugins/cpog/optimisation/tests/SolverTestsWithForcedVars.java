...
public class SolverTestsWithForcedVars 
{
	...
	private CpogSolver createSolver(int[] levels) {
		return new DefaultCpogSolver<BooleanFormula<FreeVariable>>(new Optimiser<FreeVariable, OneHotIntBooleanFormula<FreeVariable>>(FreeVariable.constructor, new OneHotNumberProvider<FreeVariable>(FreeVariable.constructor), levels), CleverCnfGenerator.create(FreeVariable.emptyConstructor));
	}

	private CpogSolver createSolver() {
		return new DefaultCpogSolver<BooleanFormula<FreeVariable>>(new Optimiser<FreeVariable, OneHotIntBooleanFormula<FreeVariable>>(FreeVariable.constructor, new OneHotNumberProvider<FreeVariable>(FreeVariable.constructor)), CleverCnfGenerator.create(FreeVariable.emptyConstructor));
	}

	private void printSolution(CpogEncoding<FreeVariable> solution) {
		if(solution == null)
		...
		else 
		{
			...
			{
				System.out.println(FormulaToString.toString(VariableReplacer.replace(FreeVariable.labelGetter, functions.get(i))));
			}
		}
	}
}
