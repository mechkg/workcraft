...
public class CleverCnfGeneratorTests {
	...
	public void qwe()
	{
		final CleverCnfGenerator<FreeVariable> gen = CleverCnfGenerator.create(FreeVariable.emptyConstructor);
		
		BooleanFormula<FreeVariable> var1 = var(new FreeVariable("x"));
		BooleanFormula<FreeVariable> var2 = var(new FreeVariable("y"));
		System.out.println(gen.generateCnf(or(or(and(var1, var2), and(var2, var1)), and(var1, var2))).toString(new HumanReadableCnfPrinter<FreeVariable>(FreeVariable.labelGetter)));
	}
}
