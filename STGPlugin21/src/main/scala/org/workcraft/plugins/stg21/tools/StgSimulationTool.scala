...
object Sim {
    def getTableCellRenderer (net : MathStg) : (String, Boolean) => Component = (transitionName, isActive) => {
      ...
      val t = SimHelper.transitionByName(net, transitionName)(0)
      
      net.transitions(t)._1 match {
    ...
    def createSimulationTool(visualStg : VisualStg) : IO[ToolEnvironment => ModelEditorTool] = IO.ioPure.pure {
