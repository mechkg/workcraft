...
object EditorUIs {
  ...
  implicit val integerEditorUI : EditorUI[Integer] = EditorUI (name => x => IntegerProperty(name, x))
  implicit val intEditorUI : EditorUI[Int] = EditorUI (name => x => IntegerProperty(name, x.applyIso[Integer](x => x, x => x)))
