...
public class SolutionReader 
{
	...
	private static final class BooleanSolutionImplementation<Var> implements BooleanSolution<Var>
	{
		private final Map<Var, Boolean> results;

		private BooleanSolutionImplementation(Map<Var, Boolean> results) {
		...
		@Override
		public boolean getSolution(Var variable) {
			Boolean result = results.get(variable);
			if(result == null)
				throw new RuntimeException("No solution for variable: " + variable.toString());
			return result;
		}
	}

	public static <Var> BooleanSolution<Var> readSolution(CnfTask<Var> task, String solution) {
		Map<String, Var> vars = task.getVars();
		...
		final Map<Var, Boolean> results = new HashMap<Var, Boolean>(); 
		...
		return new BooleanSolutionImplementation<Var>(results);
	}
