...
public class StgMathNodeRemovalTest {
	...
	public void mathNodeRemovalTest() throws InvalidConnectionException {
		...
		DummyTransition mathTransition = transition.getReferencedTransition();
		Assert.assertTrue(eval(stg.getRoot().children()).contains(mathTransition));
		visualStg.remove(arc);
		Assert.assertFalse(eval(stg.getRoot().children()).contains(mathTransition));
	}
}
