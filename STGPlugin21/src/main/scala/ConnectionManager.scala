      ...
      def connect(node1 : StgConnectable, node2 : StgConnectable) : Action = {
        (node1, node2) match {
          case (NodeConnectable(n1), NodeConnectable(n2)) => (n1, n2) match {
            case (ExplicitPlaceNode(p), TransitionNode(t)) => addConnection(ConsumingArc(p, t))
            case (TransitionNode(t), ExplicitPlaceNode(p)) => addConnection(ProducingArc(t, p))
            case (TransitionNode(t1), TransitionNode(t2)) => addConnection(ImplicitPlaceArc(t1, t2, 0))
            case (ExplicitPlaceNode(_), ExplicitPlaceNode(_)) => throw new InvalidConnectionException("Arcs between places are not allowed")
          }
